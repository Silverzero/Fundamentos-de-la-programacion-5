...
bool conecta4(tConecta4 const& juego, int fil, int col);  // Comprueba si en la posición dada hay 4 fichas conectadas en alguna dirección
int enLinea(tConecta4 const& juego, int fil, int col, int dir);  // Número de fichas conectadas en la dirección y posición dadas
//-------------------------------------------------------------------------------
inline tTurno cambiar(tTurno t) {   // Cambia el turno
   return (t == Jugador1) ? Jugador2 : Jugador1;
}
//-------------------------------------------------------------------------------
inline bool enRango(int f, int c) {  // Comprueba si la posición dada es válida para el tablero
   return 0 <= c && c < COLS && 0 <= f && f < FILAS;
}
//-------------------------------------------------------------------------------
inline bool sePuede(tConecta4 const& juego, int col) {  // Comprueba si la columna dada es una jugada posible
...
bool aplicarJugada(tConecta4 & juego, int col) {
   if (juego.estado == Jugando && sePuede(juego, col)) {
      ...
      ++juego.fichas;
      if (conecta4(juego, pos, col)) juego.estado = Ganador;
      else if (juego.fichas == FILAS*COLS) juego.estado = Bloqueo;
      else juego.turno = cambiar(juego.turno);
      return true;
   } 
   else return false;
}
